...
class TestPermutation(unittest.TestCase):
	def test_equivalence_of_parity_methods(self):
		...
		parities = [
			parity(permutation, method) for method in list(ParityMethod)
		]
		self.assertEqual(len(set(parities)), 1)

	@parameterized.expand([(method, permutation, desired)
		for method in list(ParityMethod)
		for permutation, desired in zip([
		range(0, 10),
		[3, 4, 5, 2, 1],
		], [
		1,
	...
	def test_inversion_vector(self, permutation, desired):
		self.assertEqual(inversion_vector(permutation), desired, msg="{0}".format((permutation,desired)))
	...
	def test_left_inversion_count(self, permutation, desired):
		self.assertEqual(left_inversion_count(permutation), desired, msg="{0}".format((permutation,desired)))
	...
	def test_right_inversion_count(self, permutation, desired):
		self.assertEqual(right_inversion_count(permutation), desired, msg="{0}".format((permutation,desired)))
	...
	def test_nontcontiguous_sequence_raises_exception(self, method,
		not_permutation):
