...
from mathematics.tools.presentation.graphical import *

from ... import name_func, user_verdict
...
class TestLatexWalkerText:
    ...
    def test_LatexWalker_unicode(self, traversal, presentation):
        ...
        abelian_group = "A"
        plus = Operation(
            abelian_group, abelian_group, codomain=abelian_group, symbol="+", algebraic_structure=abelian_group,
        )
        scalprod = Operation(
            real_values, abelian_group, codomain=abelian_group, symbol=r" \cdot ", algebraic_structure=vector_space,
        )
        result_graph = plus(scalprod("r_0", "a_0"), scalprod("r_1", "a_1"))
        result = LatexWalker(traversal)(result_graph)

        verdict = user_verdict.get_textverdict(sys._getframe().f_code.co_name, result, timeout=10)
        assert verdict
...
class TestLatexWalkerGraphics:
    @pytest.mark.parametrize(
        ...
        [
            (traversal, presentation)
            for traversal in list(LatexWalker.Traversal)
            for presentation in (LatexNode.Presentation.URL_IMG_PNG, LatexNode.Presentation.TEXT,)
        ],
        ids=name_func,
    )
    @pytest.mark.slow
    def test_latexwalker_image(self, traversal, presentation):
        ...
        result_graph = forall("r", "a_0", "a_1", plus(scalprod("r", "a_0"), scalprod("r", "a_1")))

        LatexNode.presentation = presentation
        if presentation in (LatexNode.Presentation.TEXT, LatexNode.Presentation.UNICODE_APPROXIMATION,):
            ...
            verdict = user_verdict.get_mediaverdict(sys._getframe().f_code.co_name, result_image, timeout=10)
            result_image.unlink()
            assert verdict
        else:
