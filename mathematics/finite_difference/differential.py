...
def _flow(direction, dt):
    def flow_for(function):
        return (function if isinstance(function, _Pointwise) else _Pointwise(function)).after(
            lambda point: point
            + dt * (direction if isinstance(direction, _Pointwise) else _Pointwise(direction))(point)
        )
...
def _finite_forward_difference(direction, dt):
    def finite_forward_difference_for(function):
        return (_flow(direction, dt)(function) - _flow(direction, 0)(function)) / _Pointwise(dt)
...
def _finite_central_difference(direction, dt):
    def finite_central_difference_for(function):
        return (
            _flow(direction, dt)(function) - 2.0 * _flow(direction, 0)(function) + _flow(direction, -dt)(function)
        ) / _Pointwise(dt ** 2)
...
def __dt_heuristic(direction, function, derivative_order, approximation_order, *args):
	...
	"""
    max_float_spacing_in_codomain = max((_np.spacing(fi)) for fi in _np.ravel(function(*args)))

    recommended_dt = abs(max_float_spacing_in_codomain) ** (1.0 / (derivative_order + approximation_order))
    trial_dts = [recommended_dt * (10.0 ** exponent) for exponent in range(0, 10)]
    trial_dfs = [_finite_forward_difference(direction, dt)(function)(*args) for dt in trial_dts]
    trial_dfs_rounded = [tuple(float(format(val, ".3e")) for val in _np.ravel(trial_df)) for trial_df in trial_dfs]
...
def __finite_forward_difference_heuristic(direction):
    def finite_forward_difference_heuristic_for(function):
        return lambda *args: _finite_forward_difference(direction, __dt_heuristic(direction, function, 1, 1, *args))(
            function
        )(*args)
...
def __finite_central_difference_heuristic(direction):
    def finite_central_difference_heuristic_for(function):
        return lambda *args: _finite_central_difference(direction, __dt_heuristic(direction, function, 1, 2, *args))(
            function
        )(*args)
...
def directional_derivative(*directions):
    ...
    if len(directions) == 1:
        return lambda function: __finite_forward_difference_heuristic(directions[-1])(function)
