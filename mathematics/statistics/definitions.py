...
class ProbabilitySpace(MeasureSpace):
    def __init__(self, domain: MeasurableSpace, measure):
        ...
        assert (
            self.measure_space.measure()(self.measure_space.measurable_space().set())
            == 1
        )


def function_is_measurable(
    domain: MeasurableSpace, function, codomain: MeasurableSpace
):
    return all(
        preimage(domain.set(), function, codomain_measurable_set)
        in domain.sigma_algebra_on_set()
        for codomain_measurable_set in codomain.measurable_sets()
    )


def integral(
    measurable_integration_region, measure_space: MeasureSpace, *level_set_value_pair
):
    return sum(
        value
        * measure_space.measure()(measurable_integration_region.intersection(level_set))
...
def lebesgue_integral_limit_sequence(
    ...
    for measurable_codomain_subset in codomain_partitioned_into_measurable_subsets:
        measurable_codomain_subset_representative_element_chooser(
            measurable_codomain_subset
        ) * preimage_mapping(measurable_codomain_subset)
