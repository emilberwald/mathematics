pipeline {
    ...
    stages {
        stage('Setup')
        {
            steps {
                cleanWs()
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'mathematics']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/emilberwald/mathematics.git']]])
            }
        }
        stage('Analyze')
        {
            steps
            {
                ...
                dir('mathematics/src') {
                    sh label: 'Pylint', script: 'python3 -m pylint --msg-template=\'{path}:{line}: [{msg_id}, {obj}] {msg} ({symbol})\' mathematics | tee ${WORKSPACE}/pylint.log'
                }
